...
}, [leftExpression, rightExpression], [...(leftExpression ? leftExpression.tokens : []), operatorToken, ...(rightExpression ? rightExpression.tokens : [])]])([takeExpression, arg => (([result, tokens]) => {
  if (result === null) {
    throw new Error('Expected token was not found');
  }

  return [result, tokens];
})(arg.length > 0 && isToken(arg[0]) && (!undefined || arg[0].kind === undefined) ? [arg[0], arg.slice(1)] : [null, arg]), takeExpression].reduce(([results, remainingTokens], processor, index) => {
  const [result, newRemainingTokens] = processor(remainingTokens);
  return [[...results, result], newRemainingTokens];
}, [[], arg]));
